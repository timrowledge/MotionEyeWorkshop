...
class PWM:
   ...
   def read_pulse_width(self, channel):
      (nb, data) = self.pi.i2c_read_i2c_block_data(self.h, self._LED0_ON_L+4*channel, 4)
      start = int(data[0]) + int(data[1]<<8)
      stop  = int(data[2]) + int(data[3]<<8)
      return int((stop - start) / 4096.0 * self._pulse_width);
...
if __name__ == "__main__":
   ...
   import PCA9685_pantilt as PCA9685
   import pigpio
   import sys
   ...
   if center == 1:
      ...
      print 'move %d', move
      p = pwm.read_pulse_width(channel)
      p = p + move
      if p > 2400:
         p = 2400
      if p < 600:
         p = 600
      pwm.set_pulse_width(channel, p) 
